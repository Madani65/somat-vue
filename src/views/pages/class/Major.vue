<template>
  <div>
    <header-school :title="$vuetify.lang.t('$vuetify.class.major.list')" />
    <v-card
      elevation="1"
    >
      <v-card-text>
          ...
          <v-row>
            <v-col cols="12" class="pa-0">
              <v-data-table
                ...
                :server-items-length="totalMajors"
                :footer-props="propsFooterTable"
                class="header-table-blue"
                :loading="loading"
                :loading-text="$vuetify.lang.t('$vuetify.loadingdata')"
              >
                <template v-slot:[`item.major_code`]="props">
                  <v-text-field
                    v-if="props.item.updated"
                    v-model="props.item.major_code"
                    dense
                    filled
                    hide-details="auto"
                    class="ma-1"
                    required
                    :error-messages="getErrorMessage(`classmajor.${props.index}.major_code`)"
                  />
                ...
                <template v-slot:[`item.name`]="props">
                  <v-text-field
                    ...
                    class="ma-1"
                    required
                    :error-messages="getErrorMessage(`classmajor.${props.index}.name`)"
                  />
                  <span v-else>{{ props.item.name }}</span>
                </template>
                <template v-slot:[`item.description`]="props">
                  <v-textarea
                    ...
                    rows="1"
                    class="ma-1"
                    :error-messages="getErrorMessage(`classmajor.${props.index}.description`)"
                  />
...
export default {
  ...
  computed: {
    ...
    ...mapState( 'majors', ['majors', 'major', 'totalMajors', 'totalPageMajors', 'beforeSaveMajors']),
    ...mapGetters( 'global', ['getErrorMessage']),
    headers(){
      return [
        { text: this.$vuetify.lang.t('$vuetify.class.major.code'), value: 'major_code', width: '200', sortable: true },
        { text: this.$vuetify.lang.t('$vuetify.class.major.name'), value: 'name', width: '200', sortable: true },
        { text: this.$vuetify.lang.t('$vuetify.description'), value: 'description', sortable: true },
        { text: this.$vuetify.lang.t('$vuetify.status'), value: 'active_flag', width: '180', sortable: true },
  ...
  methods: {
    get(){
      this.loading = true
      this.$store.dispatch('majors/get', { 'options' : this.options }).then( () => {
        this.loading = false
      }).catch(() => {
        this.loading = false
      })
    ...
    doSave(){
      this.loading = true
      this.$store.dispatch('majors/save').then( () => {
    ...
    addnew(){
      this.$store.commit('majors/PUSH_BLANK_DATA')
    },
    doDelete(data){
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$store.dispatch('majors/delete', data.item).then( (resp) => {
            this.loading = false
    ...
    doCancel(data){
      this.$store.commit("global/clearErrors")
      if(data.item.class_major_id) {
        let obj = this.beforeSaveMajors[data.index]
        Object.keys(data.item).forEach(function(key) { data.item[key] = obj[key] })
      }
      else
        this.$store.commit('majors/DELETED_BY_IDX', data.index)
    }
  }
}
</script>
