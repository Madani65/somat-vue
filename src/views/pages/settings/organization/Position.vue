            ...
            >
              <template v-slot:[`item.name`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`position.${props.index}.name`)"
                />
              ...
              <template v-slot:[`item.description`]="props">
                <v-textarea
                  ...
                  rows="1"
                  class="ma-1"
                  :error-messages="getErrorMessage(`position.${props.index}.description`)"
                />
              ...
              <template v-slot:[`item.effective_start_date`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`position.${props.index}.effective_start_date`)"
              ...
              <template v-slot:[`item.effective_end_date`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`position.${props.index}.effective_end_date`)"
...
export default {
  ...
  computed: {
    ...
    ...mapState( 'positions', ['positions', 'position', 'totalPositions', 'totalPagePositions']),
    ...mapGetters( 'global', ['getErrorMessage']),
    headers(){
      return [
        { text: this.$vuetify.lang.t('$vuetify.organization.position.title'), value: 'name', width: '200', sortable: true },
    ...
    selectDate: {
      get: function() {
        if( this.selectedDate.index in this.positions ) return this.positions[this.selectedDate.index][this.selectedDate.name]
        else return null
      },
      set: function(val) {
        this.positions[this.selectedDate.index][this.selectedDate.name] = val
      }
    }
  },
  ...
  watch: {
    ...
    itemSelected(){
      if(this.itemSelected.length == 1) this.$store.commit("positions/SET_SELECTED_ITEM", this.itemSelected[0])
      else this.$store.commit("positions/SET_SELECTED_BLANK")
    },
    selectedOrganization(){
      if(this.selectedOrganization)this.get()
      else {
        this.$store.commit('positions/CLEAR_DATA')
        this.$store.commit('jobs/CLEAR_DATA')
      }
    }
  },
  ...
  methods: {
    ...
    get(){
      this.loading = true
      this.$store.dispatch('positions/get', { 'options' : this.options }).then( (resp) => {
        this.loading = false
    ...
    doSave(){
      this.loading = true
      this.$store.dispatch('positions/save').then( (resp) => {
    ...
    addnew(){
      this.$store.commit('positions/PUSH_BLANK_DATA', this.selectedOrganization.organization_id)
    },
    doDelete(data){
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$store.dispatch('positions/delete', data.item).then( (resp) => {
