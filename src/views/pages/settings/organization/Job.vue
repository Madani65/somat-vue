            ...
            >
              <template v-slot:[`item.name`]="props">
                <v-text-field
                  ...
                  v-model="props.item.job_name"
                  dense
                  filled
                  hide-details="auto"
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`job.${props.index}.job_name`)"
                />
                <span v-else>{{ props.item.job_name }}</span>
              </template>
              <template v-slot:[`item.description`]="props">
                <v-textarea
                  ...
                  rows="1"
                  class="ma-1"
                  :error-messages="getErrorMessage(`job.${props.index}.description`)"
                />
              ...
              <template v-slot:[`item.grade`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`job.${props.index}.grade`)"
                />
              ...
              <template v-slot:[`item.effective_start_date`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`job.${props.index}.effective_start_date`)"
              ...
              <template v-slot:[`item.effective_end_date`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`job.${props.index}.effective_end_date`)"
...
export default {
  ...
  computed: {
    ...
    ...mapState( 'jobs', ['jobs', 'position', 'totalJobs', 'totalPageJobs']),
    ...mapGetters( 'global', ['getErrorMessage']),
    headers(){
      return [
        { text: this.$vuetify.lang.t('$vuetify.organization.job.title'), value: 'name', width: '200', sortable: true },
        { text: this.$vuetify.lang.t('$vuetify.description'), value: 'description', sortable: true },
        { text: this.$vuetify.lang.t('$vuetify.grade'), value: 'grade', sortable: true },
    ...
    selectDate: {
      get: function() {
        if( this.selectedDate.index in this.jobs ) return this.jobs[this.selectedDate.index][this.selectedDate.name]
        else return null
      },
      set: function(val) {
        this.jobs[this.selectedDate.index][this.selectedDate.name] = val
      }
    }
  },
  ...
  watch: {
    ...
    selectedPosition(){
      if(this.selectedPosition)this.get()
      else this.$store.commit('jobs/CLEAR_DATA')
    }
  },
  ...
  methods: {
    ...
    get(){
      this.loading = true
      this.$store.dispatch('jobs/get', { 'options' : this.options }).then( (resp) => {
        this.loading = false
    ...
    doSave(){
      this.loading = true
      this.$store.dispatch('jobs/save').then( (resp) => {
    ...
    addnew(){
      this.$store.commit('jobs/PUSH_BLANK_DATA', this.selectedPosition.position_id)
    },
    doDelete(data){
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$store.dispatch('jobs/delete', data.item).then( (resp) => {
    ...
    doCancel(data){
      ...
      if(data.item.job_id){
        ...
        data.item.job_name = this.origItems[data.index].job_name
        data.item.description = this.origItems[data.index].description
        data.item.grade = this.origItems[data.index].grade
        data.item.updated = false
      }
      else
        this.$store.commit('jobs/DELETED_BY_IDX', data.index)
    },
