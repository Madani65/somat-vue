        ...
        <v-row>
          <v-col cols="12" class="pa-0">
            <v-data-table
              ...
              :server-items-length="totallessonCategories"
            ...
            >
              <template v-slot:[`item.name`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`category.${props.index}.name`)"
                />
...
export default {
  ...
  computed: {
    ...
    ...mapState( 'lessoncategory', ['lessonCategories', 'lessonCategory', 'totallessonCategories', 'totalPagelessonCategories']),
    ...mapGetters( 'global', ['getErrorMessage']),
    headers(){
      return [
        { text: this.$vuetify.lang.t('$vuetify.setup.category.lesson.title'), value: 'name', width: '200', sortable: true },
    ...
    shouldBeSave(){
      return this.lessonCategories.filter( function(item) {
        return item.updated
      }).length
    }
  },
  ...
  methods: {
    ...
    get(){
      this.loading = true
      this.$store.commit('lessoncategory/CLEAR_DATA')
      this.$store.dispatch('lessoncategory/get', { 'options' : this.options }).then( (resp) => {
        this.loading = false
    ...
    addnew(){
      this.$store.commit('lessoncategory/PUSH_BLANK_DATA')
    },
    doDelete(data){
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$store.dispatch('lessoncategory/delete', data.item).then( (resp) => {
    ...
    doCancel(data){
      this.$store.commit("global/clearErrors")
      if(data.item.lesson_category_id){
        ...
        this.$store.commit('lessoncategory/DELETED_BY_IDX', data.index)
    },
    doSave(){
      this.loading = true
      this.$store.dispatch('lessoncategory/save').then( (resp) => {
