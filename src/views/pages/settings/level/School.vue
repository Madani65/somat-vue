        ...
        <v-row>
          <v-col cols="12" class="pa-0">
            <v-data-table
              ...
              :server-items-length="totalSchoolLevels"
            ...
            >
              <template v-slot:[`item.name`]="props">
                <v-text-field
                  ...
                  class="ma-1"
                  required
                  :error-messages="getErrorMessageByUUID('schoollevel',props.item.uuid,'name', shouldBeSave)"
                />
              ...
              <template v-slot:[`item.description`]="props">
                <v-textarea
                  ...
                  rows="1"
                  class="ma-1"
                  :error-messages="getErrorMessage(`schoollevel.${props.index}.description`)"
                />
...
export default {
  ...
  computed: {
    ...
    ...mapState( 'schoollevel', ['schoolLevels', 'schoolLevel', 'totalSchoolLevels', 'totalPageSchoolLevels', 'beforeSaveSchoolLevels']),
    ...mapGetters( 'global', ['getErrorMessage', 'getErrorMessageByUUID']),
    headers(){
      return [
        { text: this.$vuetify.lang.t('$vuetify.school.level.title'), value: 'name', width: '200', sortable: true },
    ...
    shouldBeSave(){
      return this.schoolLevels.filter( function(item) {
        return item.updated
      })
    }
  },
  ...
  methods: {
    get(){
      this.loading = true
      this.$store.dispatch('schoollevel/get', { 'options' : this.options }).then( () => {
        this.loading = false
      }).catch(() => {
        this.loading = false
      })
    ...
    doSave(){
      this.loading = true
      this.$store.dispatch('schoollevel/save').then( () => {
    ...
    addnew(){
      this.$store.commit('schoollevel/PUSH_BLANK_DATA')
    },
    doDelete(data){
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$store.dispatch('schoollevel/delete', data.item).then( (resp) => {
            this.loading = false
    ...
    doCancel(data){
      this.$store.commit("global/clearErrors")
      if(data.item.school_level_id){
        let obj = this.beforeSaveSchoolLevels[data.index]
        Object.keys(data.item).forEach(function(key) { data.item[key] = obj[key] })
      }
      else
        this.$store.commit('schoollevel/DELETED_BY_IDX', data.index)
    }
  }
}
</script>
