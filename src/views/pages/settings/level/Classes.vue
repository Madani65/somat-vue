      ...
      <v-container>
        <v-row>
          <v-col cols="12" md="6" class="px-0">
            <api-select 
              v-model="selectSchoolLevel"
              dispatch-name="schoollevel/getActive"
              :label="$vuetify.lang.t('$vuetify.school.level.title')"
              item-text="name"
              return-object
              :hint="$vuetify.lang.t('$vuetify.pick_before_add')"
            />
          </v-col>
        </v-row>
        <v-row class="pt-4 pb-3">
          <v-btn
            small
            :disabled="loading || !selectSchoolLevel"
        ...
        <v-row>
          <v-col cols="12" class="pa-0">
            <v-data-table
              ...
              :server-items-length="totalClassLevels"
              :footer-props="propsFooterTable"
              class="header-table-blue"
              :loading="loading"
              :loading-text="$vuetify.lang.t('$vuetify.loadingdata')"
            >
              <template v-slot:[`item.class_level`]="props">
                <v-text-field
                  v-if="props.item.updated"
                  v-model="props.item.class_level"
                  dense
                  filled
                  hide-details="auto"
                  class="ma-1"
                  required
                  :error-messages="getErrorMessage(`classlevel.${props.index}.class_level`)"
                />
                <span v-else>{{ props.item.class_level }}</span>
              </template>
              <template v-slot:[`item.description`]="props">
                <v-textarea
                  ...
                  rows="1"
                  class="ma-1"
                  :error-messages="getErrorMessage(`classlevel.${props.index}.description`)"
                />
...
export default {
  ...
  computed: {
    ...
    ...mapState( 'schoollevel', ['schoolLevels', 'selectedSchoolLevel']),
    ...mapState( 'classlevel', ['classLevels', 'classLevel', 'totalClassLevels', 'totalPageClassLevels','beforeSaveClassLevels']),
    ...mapGetters( 'global', ['getErrorMessage']),
    headers(){
      return [
        { text: this.$vuetify.lang.t('$vuetify.school.level.class'), value: 'class_level', width: '200', sortable: true },
    ...
    shouldBeSave(){
      return this.classLevels.filter( function(item) {
        return item.updated
      }).length
    },
    selectSchoolLevel: {
      ...
      set: function(val){
        this.$store.commit("schoollevel/SET_SELECTED_ITEM", val)
      }
    }
  },
  ...
  watch: {
    selectedSchoolLevel(){
      if(this.selectedSchoolLevel)this.get()
      else {
        this.$store.commit('classlevel/CLEAR_DATA')
      }
    }
  },

  created(){
    this.getListSchoolLevel()
    this.$store.commit("schoollevel/SET_SELECTED_BLANK")
  },

  methods: {
    getListSchoolLevel(refreshList){
      this.loadingLOV = true
      this.$store.dispatch('schoollevel/getActive').then( (resp) =>{
        this.listSchoolLevel = resp.data
        if (refreshList && this.selectSchoolLevel) this.get()
      }).finally( () => {
        this.loadingLOV = false
      })
    },
    get(){
      this.loading = true
      this.$store.dispatch('classlevel/get', { 'options' : this.options }).then( () => {
        this.loading = false
      }).catch(() => {
        this.loading = false
      })
    ...
    doSave(){
      this.loading = true
      this.$store.dispatch('classlevel/save').then( () => {
    ...
    addnew(){
      this.$store.commit('classlevel/PUSH_BLANK_DATA')
    },
    doDelete(data){
      this.$swal({
        ...
        if (result.value) {
          this.loading = true
          this.$store.dispatch('classlevel/delete', data.item).then( (resp) => {
    ...
    doCancel(data){
      this.$store.commit("global/clearErrors")
      if(data.item.class_level_id){
        let obj = this.beforeSaveClassLevels[data.index]
        Object.keys(data.item).forEach(function(key) { data.item[key] = obj[key] })
      }
      else
        this.$store.commit('classlevel/DELETED_BY_IDX', data.index)
    }
  }
}
</script>
